...
impl StreamingInfo {
    ...
    pub fn load_data<'a>(&self, fs: &'a UnityFsFile<'a>) -> crate::Result<&'a [u8]> {
        let path = self.path.split('/').last().ok_or(Error::InvalidData("streaming data path incorrect"))?.as_bytes();
        let node = fs.entries().find(|e| e.path_raw() == path).ok_or(Error::InvalidData("streaming data file not found"))?;

        let offset = usize::from_int(self.offset.0)?;
        let size = usize::from_int(self.size)?;

        let slice = node.read_raw()?
            .get(offset..).ok_or(Error::InvalidData("streaming data offset out of bounds"))?
            .get(..size).ok_or(Error::InvalidData("streaming data size out of bounds"))?;

        Ok(slice)
    }

    pub fn load_data_or_else<'t, 'fs: 't>(&self, fs: &'fs UnityFsFile<'fs>, fallback: impl FnOnce() -> &'t [u8]) -> crate::Result<&'t [u8]> {
